...
<?php if($display_footer_top || $display_footer_text) { ?>
	...
	<?php 
	// checking on which pages should be used the banner
	if (is_front_page() && get_field('disable') == 'No' || is_checkout() && get_field('disable') == 'No' ) :

		$cookie_expiried = get_field('cookie_expiried', 'option');
		$show_on_delay = get_field('show_on_delay', 'option');
		$show_once_per_session = get_field('show_once_per_session', 'option');
		$delay = get_field('delay', 'option');

		$mainTitle = get_field('heading', 'option');
		$subTitle = get_field('paragraph', 'option');
		$paragraphTitle = get_field('paragraph_normal', 'option');

		$codeParagraph = get_field('code_paragraph', 'option');
		$codeName = get_field('code', 'option');

		$offerTitle = get_field('offer_paragraph_', 'option');
		$offerDate = get_field('offer_ends_date', 'option');
		$subOfferTitle = get_field('sub_paragraph', 'option');
		$buttonLink = get_field('button', 'option');

		$termsAndConditions = get_field('terms_and_conditions', 'option');
		...
		<div id="bio_ep">
			<div class="flexCnt">
				<div class="boxTitle">
					<h2><?= $mainTitle ?></h2>
					<p class="paragraphColor"><?= $subTitle ?></p>
					<p class="paragraph"><?= $paragraphTitle ?></p>

				</div>
				<div class="boxCode">
					<p class="paragraph"><?= $codeParagraph ?> <span class="code"><?= $codeName ?></span></p>
				</div>
				<div class="boxOffer">
					<p class="paragraph">
						<?= $offerTitle ?> <span class="date"><?= $offerDate ?></span>
					</p>
					<p class="sub"><?= $subOfferTitle ?></p>
					<a target="_blank" href="<?= $buttonLink ?>">SHOP NOW</a>
				</div>
				<div class="boxConditions">
	    ...
	    <script type="text/javascript">
	        bioEp.init({
				cookieExp: <?= $cookie_expiried ?>,
				showOnDelay: <?= $show_on_delay ?>,
				showOncePerSession: <?= $show_once_per_session ?>,
				delay: <?= $delay ?>,
				shown: true,
